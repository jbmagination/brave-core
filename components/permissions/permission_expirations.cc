...
void PermissionExpirations::UpdateExpirationsPref(
  ...
  for (const auto& expiration_key : expiration_keys) {
    ...
    const std::string& key = expiration_key.ToPrefKey();
    if (key_expirations_it == key_expirations_map.end() ||
      ...
      key_expirations_val->RemovePath(
          base::StrCat({content_type_name, ".", key}), nullptr);
    } else {
...
void PermissionExpirations::ReadExpirationsFromPrefs() {
  ...
  for (const auto& type_expirations_val :
    ...
    for (const auto& key_expirations_val :
      ...
      key_expirations_map.emplace(PermissionExpirationKey::FromPrefKey(key_str),
